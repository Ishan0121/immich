...
enum class UploadApiGroup(val raw: Int) {
  MANUAL(0),
  BACKUP(1),
  BACKUP_LIVE_PHOTO(2),
  IMAGE(3),
  ...
  companion object {
    fun ofRaw(raw: Int): UploadApiGroup? {
      return values().firstOrNull { it.raw == raw }
    }
  }
}
...
)
 {
  companion object {
    fun fromList(pigeonVar_list: List<Any?>): UploadApiTaskStatus {
      ...
      val group = pigeonVar_list[2] as UploadApiGroup
      val status = pigeonVar_list[3] as UploadApiStatus
      val errorCode = pigeonVar_list[4] as UploadApiErrorCode?
      val httpStatusCode = pigeonVar_list[5] as Long?
      return UploadApiTaskStatus(id, filename, group, status, errorCode, httpStatusCode)
    }
  }
  fun toList(): List<Any?> {
    return listOf(
...
interface UploadApi {
  ...
  fun enqueue(localIds: List<String>, callback: (Result<Unit>) -> Unit)
  ...
  companion object {
    ...
    fun setUp(binaryMessenger: BinaryMessenger, api: UploadApi?, messageChannelSuffix: String = "") {
      ...
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.immich_mobile.UploadApi.enqueue$separatedMessageChannelSuffix", codec)
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            val args = message as List<Any?>
            val localIdsArg = args[0] as List<String>
            api.enqueue(localIdsArg) { result: Result<Unit> ->
              val error = result.exceptionOrNull()
              if (error != null) {
                reply.reply(UploadTaskPigeonUtils.wrapError(error))
              } else {
...
class UploadFlutterApi(private val binaryMessenger: BinaryMessenger, private val messageChannelSuffix: String = "") {
  companion object {
    /** The codec used by UploadFlutterApi. */
    val codec: MessageCodec<Any?> by lazy {
      UploadTaskPigeonCodec()
    }
  }
  fun onTaskProgress(progressArg: UploadApiTaskProgress, callback: (Result<Unit>) -> Unit)
{
    val separatedMessageChannelSuffix = if (messageChannelSuffix.isNotEmpty()) ".$messageChannelSuffix" else ""
    val channelName = "dev.flutter.pigeon.immich_mobile.UploadFlutterApi.onTaskProgress$separatedMessageChannelSuffix"
    ...
    channel.send(listOf(progressArg)) {
      if (it is List<*>) {
        ...
        callback(Result.failure(UploadTaskPigeonUtils.createConnectionError(channelName)))
      } 
    }
  }
  fun onTaskStatus(statusArg: UploadApiTaskStatus, callback: (Result<Unit>) -> Unit)
{
    val separatedMessageChannelSuffix = if (messageChannelSuffix.isNotEmpty()) ".$messageChannelSuffix" else ""
    val channelName = "dev.flutter.pigeon.immich_mobile.UploadFlutterApi.onTaskStatus$separatedMessageChannelSuffix"
    val channel = BasicMessageChannel<Any?>(binaryMessenger, channelName, codec)
