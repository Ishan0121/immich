...
class UploadApiDelegate: NSObject, URLSessionDataDelegate, URLSessionTaskDelegate {
  ...
  init(db: DatabasePool, flutterApi: UploadFlutterApi) {
    self.db = db
  ...
  func urlSession(_ session: URLSession, task: URLSessionTask, didCompleteWithError error: Error?) {
    Task {
        ...
        let httpResponse = task.response as? HTTPURLResponse
      {
        ...
        case 400..<500:
          dPrint("Response \(httpResponse.statusCode): \(String(data: responseData, encoding: .utf8) ?? "No response body")")
  ...
  func urlSession(
    ...
    if let previousSpeed = state.currentSpeed {
      let alpha = 0.4  // 40% new speed, 60% old speed
      state.currentSpeed = alpha * instantSpeed + (1 - alpha) * previousSpeed
    } else {
    ...
    DispatchQueue.main.async {
      let progress = UploadApiTaskProgress(
        id: sessionTaskId,
        progress: Double(totalBytesSent) / Double(totalBytesExpectedToSend),
        speed: state.currentSpeed ?? 0
      )
      self.flutterApi.onTaskProgress(progress: progress, completion: { _ in })
    }
  }
  ...
  private func handleSuccess(taskId: Int64, response: UploadSuccessResponse) async {
    ...
    do {
      try await db.write { conn in
        let task = try UploadTask.update(set: { $0.status = .uploadComplete }).where({ $0.id.eq(taskId) })
