...
class UploadApiImpl: ImmichPlugin, UploadApi {
  ...
  init(flutterApi: UploadFlutterApi) {
    ...
    let delegate = UploadApiDelegate(db: db, flutterApi: flutterApi)
    ...
    self.downloadQueue = DownloadQueue(db: db, uploadQueue: uploadQueue, flutterApi: flutterApi)
  ...
  func enqueue(localIds: [String], completion: @escaping (Result<Void, any Error>) -> Void) {
    Task {
      ...
      do {
        ...
        guard !candidates.isEmpty else {
          dPrint("No candidates to enqueue")
